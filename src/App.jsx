...
const StaffVisualizer = ({ notePos, showNote }) => {
...
const App = () => {
    ...
    const [config, setConfig] = useState({ mode: 'random', count: 5 });
    ...
    const startQuiz = () => {
        ...
        if (config.mode === 'lines') {
            pool = allQuestions.filter(q => q.type === 'line');
        } else if (config.mode === 'spaces') {
            pool = allQuestions.filter(q => q.type === 'space');
        } else {
            pool = [...allQuestions];
        }

        const generated = generateQuestions(pool, config.count);
                    ...
                    <div className="space-y-6">
                        <div>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                <button 
                                    onClick={() => setConfig({ ...config, mode: 'lines' })}
            ...
            <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-gradient-to-br from-cyan-50 via-blue-50 to-purple-50 font-sans">
                <div className="w-full max-w-xl">
                    ...
                    <div className="w-full bg-white/50 rounded-full h-2.5 mb-6 backdrop-blur-sm">
                        <div 
                            className="bg-gradient-to-r from-cyan-500 to-blue-600 h-2.5 rounded-full transition-all duration-300 shadow" 
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
                        <div className="bg-gradient-to-br from-gray-800 to-gray-900 text-white p-6 text-center relative overflow-hidden">
                            ...
                            {!showAnswer && (
                                <p className="mt-4 text-cyan-300 italic text-sm animate-pulse font-medium relative z-10">
                        ...
                        <div className="p-6">
                            <StaffVisualizer notePos={question.pos} showNote={showAnswer} />
                            ...
                            <div className="mt-6">
                                {!showAnswer ? (
                                    <button 
                                        ...
                                        className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-bold py-3.5 px-6 rounded-xl shadow-md transition active:scale-95"
                                    >
                                        Mostra Risposta
                                    </button>
                                ) : (
                                    <button 
                                        onClick={handleNext}
                                        className="w-full bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 text-white font-bold py-3.5 px-6 rounded-xl shadow-md transition flex items-center justify-center gap-2 active:scale-95"
                                    >
                                        {currentQuestionIndex < quizQuestions.length - 1 ? "Prossima" : "Concludi"} 
                    ...
                    <button 
                        onClick={resetToMenu}
                        className="mt-6 flex items-center justify-center gap-2 text-sm font-medium text-cyan-800/60 hover:text-cyan-800 transition mx-auto"
                    >
        ...
        <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-gradient-to-br from-cyan-50 via-blue-50 to-purple-50 font-sans">
            <div className="bg-white max-w-md w-full rounded-2xl shadow-xl p-8 text-center">
                ...
                <div className="space-y-3">
                    <button 
                        onClick={startQuiz}
                        className="w-full bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 text-white font-bold py-3 px-6 rounded-xl shadow transition flex items-center justify-center gap-2"
                    >
                        <RotateCcw size={18} /> Ripeti Stesso Esercizio
                    </button>
                    <button 
                        onClick={resetToMenu}
                        className="w-full bg-white border-2 border-gray-200 hover:border-gray-300 text-gray-700 font-bold py-3 px-6 rounded-xl transition flex items-center justify-center gap-2"
                    >
